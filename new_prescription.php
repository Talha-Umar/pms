...
if(isset($_POST['submit'])) {
  ...
  $ClinicalHistory = $_POST['clinical_history'];
  ...
  try {
    ...
    `next_visit_date`, `chief_complaint`, `advised`, `clinical_history`, `patient_id`, `note`, `tbill`, `pbill`, `rbill`) 
    VALUES('$visitDate', 
    nullif('$nextVisitDate', ''), 
    '$ChiefComplaint', '$Advised', '$ClinicalHistory', '$patientId','$note','$tbill','$pbill','$rbill');";
    ...
    <div class="content-wrapper">
      ...
      <section class="content">
        ...
        <div class="card card-outline card-primary rounded-0 shadow">
          ...
          <div class="card-body">
            <!-- best practices-->
            <form method="post" id="formId" target="">
              <div class="row">
          ...
          <div class="col-lg-3 col-md-3 col-sm-4 col-xs-10">
            <div class="form-group">
              ...
              id="next_visit_date" 
              data-target-input="nearest">
              <input type="text" class="form-control form-control-sm rounded-0 datetimepicker-input" data-target="#next_visit_date" name="next_visit_date" data-toggle="datetimepicker" autocomplete="off" required />
              <div class="input-group-append" 
      ...
      <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
        ...
        <input id="chief_complaint" data-target="#chief_complaint" 
  class="form-control form-control-sm rounded-0" name="chief_complaint" required="required" />
        
      </div>
      
      <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
        <label>Clinical History</label>
        <input id="clinical_history" name="clinical_history" data-target="#clinical_history" 
        class="form-control form-control-sm rounded-0" required />
      </div>

      <div class="col-lg-2 col-md-2 col-sm-6 col-xs-12">
        <label>Advised</label>
        <input id="advised" required="required"data-target="#advised" 
        name="advised" class="form-control form-control-sm rounded-0" />
      </div>
   ...
   <div class="row">
       <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <label>Note</label>
        <input id="note" name="note" class="form-control form-control-sm rounded-0" required />
  </div>
  <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <label>Total Bill</label>
        <input id="tbill" name="tbill" class="form-control form-control-sm rounded-0" required />
  </div> 
  <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <label>Payed Amount</label>
        <input id="pbill" name="pbill" class="form-control form-control-sm rounded-0" required />
  </div>
  <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
        <label>Remaining Amount</label>
        <input id="rbill" name="rbill" class="form-control form-control-sm rounded-0" disabled />
  </div>
   </div>
...
<div class="clearfix">&nbsp;</div>
  <div class="row">
    <div class="col-md-10">&nbsp;</div>
    <div class="col-md-2">
      <button type="submit" id="print_patients" 
      class="btn btn-primary btn-sm btn-flat btn-block">Generate PDF</button>
    </div>
  </div>
...
<script>
 ...
 $("#submit").click(function() {
    ...
    if(oldData==''){
      showCustomMessage('Please add the perception first');
      return false;
    }
     else{
      $('#formId').attr('target', '');
      $('#formId').attr('action', 'new_prescription.php');
      return true;
    }
  });

 $("#print_patients").click(function() {
    var oldData = $("#current_medicines_list").html();
    if(oldData==''){
      showCustomMessage('Please add the perception first');
      return false;
      ...
      $('#formId').attr('action', 'print_patients.php');
      return true;
    }
  });
     $("#pbill").change(function() {
      var tbill = $("#tbill").val();
      var pbill = $("#pbill").val();
      var rbill=tbill-pbill;
      $("#rbill").val(rbill);
     });
